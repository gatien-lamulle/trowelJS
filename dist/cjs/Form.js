...
class Form {
    ...
    create() {
        let currentContainer = this.formContainer;
        ...
        buttonSubmit.setAttribute("type", this.submit && this.submit.action ? "submit" : "button");
        ...
        if (this.submit && (this.submit.handler || (this.submit.checkers && this.submit.checkers.length > 0))) {
            buttonSubmit.addEventListener("click", () => {
                let values = [];
                Array.from(this.formContainer.querySelectorAll("input, textarea, select")).map((node) => {
                    values.push({
                        name: node.name,
                        value: node.value,
                        type: node.type,
        ...
        for (let field of this.fields) {
            ...
            if (field.fields) {
                ...
                this.addFieldToContainer(field, fieldContainer);
            }
    ...
    addFieldToContainer(field, fieldContainer) {
        ...
        if (typeof field === "string") {
            ...
            switch (field.type) {
                case "textarea":
                    ...
                    if (field.defaultValue)
                        fieldElement.innerText = field.defaultValue;
                    break;
                case "select":
            ...
            if (field.classes)
                fieldElement.classList.add(...field.classes);
            if (field.id && !(this.globalLabels === "default" || field.label))
                fieldElement.id = field.id;
            if (field.onChange)
                fieldElement.onchange = field.onChange;
            if (field.onBlur)
                fieldElement.onblur = field.onBlur;
            if (field.onFocus)
                fieldElement.onfocus = field.onFocus;
            if (labelContainer) {
                labelContainer.appendChild(fieldElement);
                fieldContainer.appendChild(labelContainer);
            }
